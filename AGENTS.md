...
Two data paths feed the same Liquid template with the same merge variables (`order_number`, `queue`, `outstanding_orders`, `updated_at`):

1. **Polling path** (production) - TRMNL polls `src/worker.ts` (Cloudflare Worker) on a schedule. The Worker does GET+POST against trmnl.com to handle the CSRF token exchange, then returns JSON.
...
Both paths duplicate the `fetchQueue()` logic (CSRF GET, then POST with token+cookie). The Worker version reads `ORDER_NUMBER` from wrangler.jsonc vars; the script version reads from `.env`.

`trmnl/markup.liquid` is the device display template - paste it into the TRMNL plugin dashboard Markup tab. `scripts/trmnl-preview.ts` serves it locally with mock data using TRMNL's CSS/JS from their CDN.

`trmnl/form-builder.yaml` defines the plugin's "about" metadata for the TRMNL plugin settings page.

## Environment

- Worker config: `ORDER_NUMBER` is set in `wrangler.jsonc` vars (not secrets)
...
- Maintainer-only operational details (account ID, plugin UUID, deployed WAF rules) live in `MAINTAINER.local.md` (gitignored)
